...
void TestTUMBundle() {
    ...
    for(int i = 0; i <= 3; i++) {
        Eigen::Vector2d curr_2d = Eigen::Vector2d(box_corners[i][0]/box_corners[i][2], box_corners[i][1]/box_corners[i][2]);
    ...
    for(int i = 1; i <= 4; i++) {
        ...
        Eigen::Vector2d curr_2d = Eigen::Vector2d(RandomGaussian(curr_3d.x()/curr_3d.z(), 0.03), RandomGaussian(curr_3d.y()/curr_3d.z(), 0.03));
